...
def apply_data_augmentation(image):
    """ Apply Gaussian blur and JPEG compression to the image for data augmentation. """
    # 将张量转换为PIL图像
    ...
    image.save(img_buffer, format='JPEG', quality=50)  # Adjust quality as needed
    img_buffer.seek(0)
...
def train(args, model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, summary_writer):
    ...
    for i, (image, label, text, fake_image_box, fake_word_pos, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        loss_nrr,loss_MAC, loss_BIC, loss_bbox, loss_giou, loss_TMG, loss_MLC = model(
            image, label, text_input, fake_image_box, fake_token_pos, alpha=alpha)

        loss = (config['loss_nrr_wgt'] * loss_nrr+ 
                config['loss_MAC_wgt'] * loss_MAC +
        ...
        metric_logger.update(loss_nrr=loss_nrr.item())
...
def main_worker(gpu, args, config):
    ...
    train_dataset, val_dataset = create_dataset(config)
         ...
         # 选择抽样比例
    # sample_fraction = 0.2  # 这里设置为20%的数据用于训练
    # indices = torch.randperm(len(train_dataset))[:int(len(train_dataset) * sample_fraction)]
    # train_dataset = Subset(train_dataset, indices) # 随机抽样
    if args.distributed:
    ...
    tokenizer = BertTokenizerFast.from_pretrained('/home/zhoujun/models')
