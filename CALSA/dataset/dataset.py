...
class DGM4_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 使用图像路径作为唯一标识符
        image_path = img_dir  # 这是相对路径，如 "DGM4/manipulation/infoswap/995762-043201-infoswap.jpg"
        ...
        # 处理pred_bboxes - 如果存在的话
        if 'pred_bboxes' in ann and ann['pred_bboxes']:
            pred_pos = torch.tensor(ann['pred_bboxes'], dtype=torch.float)
        else:
            pred_pos = torch.tensor([0, 0, 0, 0], dtype=torch.float)
...
#         pred_pos = torch.tensor(ann['pred_bboxes'], dtype=torch.float)
#         # cx_p, cy_p, w_p, h_p = self.get_bbox(ann['pred_bboxes'])
#         # pred_pos = torch.tensor([cx_p, cy_p, w_p, h_p], dtype=torch.float)
#         do_hflip = False
#         if self.is_train:
#             if rand() < 0.5:
#                 # flipped applied
...
#         return image, label, caption, fake_image_box, fake_text_pos_list, W, H, pred_pos
...
#         return image, label, caption, fake_image_box, fake_text_pos_list, W, H, image_path