...
from models.CALSA import HAMMER
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (image, label, text, fake_image_box, fake_word_pos, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        logits_real_fake, logits_multicls, output_coord, logits_tok  = model(image, label, text_input, fake_image_box, fake_token_pos,scale, is_train=False)
...
def generate_scale(image):
    """根据图像的尺寸生成 scale"""
    batch_size, _, height, width = image.size()
    scale = torch.tensor([height, width], dtype=torch.float32).repeat(batch_size, 1)
    return scale 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--text_encoder', default='.../models/bert-base-uncased')
